...
async function render() {
  ...
  const html = [
    ...
    'input#search:focus { box-shadow: 0 0 5px rgba(0,123,255,0.5); }',
    ...
    '<body>',
    '<div id="controls">',
    '  <input type="text" id="search" placeholder="Search (e.g. folder/file)..." autocomplete="off">',
    '  <div id="search-stats" class="stats"></div>',
    '  <div class="help">',
    '    <b>Ctrl+Click</b>: Select/Focus node<br>',
    '    <b>Esc</b>: Reset View / Clear Search<br>',
    '    <b>Enter</b>: Cycle through results',
    '  </div>',
    '</div>',
    ...
    '    relPath = match[1];',
    '  } else {',
    '    // Fallback for root or nodes without injected links',
    '    const plainContent = node.content.replace(/<[^>]*>/g, "").trim();',
    ...
    '}',
    'walk(root);',
    'console.log("Path map initialized with " + pathMap.size + " nodes.");',
    'diag.textContent = "Nodes: " + pathMap.size;',
    '',
    'function updateUI() {',
    ...
    '      // Try to find the link (path element) leading to this node',
    '      // Markmap structure: link usually precedes the node group',
    ...
    '}',
    '',
    'function focusNodes(paths, zoom = true) {',
    '  console.log("Focusing nodes:", paths);',
    '  const nodes = paths.map(function(p) { return pathMap.get(p); }).filter(Boolean);',
    '  if (nodes.length === 0) return;',
    '  nodes.forEach(function(n) { mm.setHighlight(n); });',
    '  if (zoom) { if (nodes.length === 1) mm.ensureVisible(nodes[0]); else mm.fit(); }',
    ...
    '    if (targets.some(function(t) { return path.toLowerCase().includes(t.toLowerCase()); })) {',
    ...
    '    searchInput.value = ""; mm.setHighlight(null); updateUI();',
    '  }',
    '});',
    '</script>',
    '</body>',
